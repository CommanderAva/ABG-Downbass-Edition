...
//	if(hasvar(src,"w_radio")) if(src:w_radio) items += src:w_radio  commenting this out since headsets go on your ears now PLEASE DON'T BE MAD KEELIN
	...
	if(hasvar(src,"w_back_gun")) if(src:w_back_gun) items += src:w_back_gun
	if(hasvar(src,"w_webbing_vest")) if(src:w_webbing_vest) items += src:w_webbing_vest
	...
	switch(slot)
		...
		if(WEAR_BACK_GUN)
			if(!src.w_back_gun)
				src.w_back_gun = W
				equipped = 1
		if(WEAR_WEBBING_VEST)
			if (src.w_webbing_vest && istype(src.w_webbing_vest, /obj/item/storage/webbing))
				var/obj/item/storage/backpack/B = src.w_webbing_vest
				if(B.contents.len < B.storage_slots && W.w_class <= B.max_w_class)
					W.loc = B
					equipped = 1
