/datum/hud/human/New(mob/living/carbon/human/owner, ui_style='icons/mob/screen1_White.dmi', ui_color = "#ffffff", ui_alpha = 255)
	...
	back_gun = new /obj/screen()
	unique_action.icon = ui_style
	unique_action.icon_state = "back_gun"
	unique_action.name = "Back gun slot"
	unique_action.screen_loc = ui_back_gun
	static_inventory += back_gun

	webbing_vest = new /obj/screen()
	unique_action.icon = ui_style
	unique_action.icon_state = "webbing_vest"
	unique_action.name = "webbing vest slot"
	unique_action.screen_loc = ui_webbing_vest
	static_inventory += webbing_vest
...
/datum/hud/human/persistant_inventory_update()
	...
	if(hud_shown)
		if(!H.species || !H.species.hud || H.species.hud.gear.len) //species without inv slots don't show items.
			...
			if(H.w_back_gun)
				H.w_back_gun.screen_loc = ui_back_gun
				H.client.screen += H.w_back_gun
			if(H.w_webbing_vest)
				H.w_webbing_vest.screen_loc = ui_webbing_vest
				H.client.screen += H.w_webbing_vest
	else
		if(H.s_store)
		...
		if(H.w_back_gun)
			H.w_back_gun.screen_loc = null
		if(H.w_webbing_vest)
			H.w_webbing_vest.screen_loc = null
